...
import io
import re
from .errors import LengthError, FormatError
...
class Record(object):
    ...
    def _read(self, f):
        ...
        try:
            ...
            print("buffer underflow at offset {0}".format(f.tell()))
        ...
        print('typeof stuff: ', record_type.decode('utf8'))
        self._type = bytearray(type_name_prefix[tnf] + record_type.decode('utf8'), 'utf8')
        print('type parsed: ', self._type)
        # self._type = bytearray((type_name_prefix[tnf] + record_type)
        # self._type = bytearray().extend(map(ord, type_name_prefix[tnf] + record_type.decode('utf8')))
        self._name = bytearray(record_name)
        self._data = bytearray(record_data)
        print("parsed {0}".format(repr(self)))

    def _write(self, f):
        """Serialize an NDEF record to a file-like object."""
        print("writing ndef record at offset {0}".format(f.tell()))

        # record_type = self.type[12:-2]
        ...
        if record_type == '':
            ...
            print("no header flags set. record type: ", record_type)

        type_length = len(self._type)
    ...
    def type(self, value):
        ...
        if (value in ('', 'unknown', 'unchanged') or
            ...
            self._type = bytearray(value, 'utf8')
        else:
            print("'{0}' is not an acceptable record type".format(value))
    ...
    @name.setter
    def name(self, value):
        self._name = bytearray(str(value), 'utf8')
    ...
    @data.setter
    def data(self, value):
        self._data = bytearray(str(value), 'utf8')
